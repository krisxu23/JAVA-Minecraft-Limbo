...
public class PacketHandler {
    ...
    public PacketHandler(LimboServer server) { this.server = server; }
    ...
    public void handle(ClientConnection conn, PacketHandshake packet) {
        ...
        Log.debug("Pinged from %s [%s]", conn.getAddress(), conn.getClientVersion());

        if (server.getConfig().getInfoForwarding().isLegacy()) {
            String[] split = packet.getServerAddress().split("\00");
            if (split.length == 3 || split.length == 4) {
                conn.setAddress(split[1]);
                conn.getGameProfile().setUuid(UuidUtil.fromString(split[2]));
            } else {
                conn.disconnectLogin("You need to connect with a proxy");
            }
        } else if (server.getConfig().getInfoForwarding().isBungeeGuard()) {
            // BungeeGuard handled in ClientConnection
        }
    }
